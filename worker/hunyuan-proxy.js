...
- 别啰嗦、别讲道理，就告诉他怎么做。`,
  ...
  // 错题分析：返回严格 JSON，便于做错题本与薄弱点统计
  cuoti: `你是一位资深老师，帮学生分析一道做错的题。仔细看图里的题目，用中文输出**严格的 JSON 对象**，不要 markdown、不要代码块、不要多余文字，只输出 JSON。字段：
{
  "subject": "学科（如 数学/物理/化学/英语/语文/生物/历史…）",
  "topic": "这道题考的核心知识点（简短，如 一元二次方程求根）",
  "tags": ["知识点标签1","标签2"],
  "question": "题干的简要文字（识别图中题目，30字内概括）",
  "answer": "正确答案",
  "analysis": "正确解题思路，分步、简明",
  "mistake": "学生在这类题最容易错的原因/易错点，一句话",
  "tip": "一句复习建议"
}
只输出上面这个 JSON，tags 给 1~3 个。`,
};
        ...
        body: JSON.stringify({
          ...
          messages: [
            { role: "system", content: system },
            {
              role: "user",
              content: [
                { type: "text", text: mode === "cuoti" ? "这是我做错的一道题，请按要求分析，只输出 JSON。" : (mode === "critique" ? "请点评这张照片。" : "我现在想拍这个画面，给我建议。") },
                { type: "image_url", image_url: { url: image } },
              ],
            },
          ],
          temperature: mode === "cuoti" ? 0.2 : 0.7,
        }),
      });
